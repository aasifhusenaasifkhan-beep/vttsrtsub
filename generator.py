...
RULES:
1. Use 'me', 'tum', 'hu', 'hai', 'kya', 'ghar' (not 'maim', 'hun', 'kyaa').
2. Keep the exact same numbering and '|||' separator format.
3. Do not change or translate any ASS tags like {\\i1} or \\N. Return them exactly as they are.
4. Keep the output strictly in the numbered format."""
    ...
    try:
        ...
        for line in res_lines:
            if "|||" in line:
                parts = line.split("|||", 1)
                try: translated_dict[int(parts[0].strip())] = parts[1].strip()
                except: pass
                
        # Fallback dictionary mapping if AI misses a line
        final_texts = []
        for i in range(len(texts)):
            final_texts.append(translated_dict.get(i, texts[i]))
        return final_texts
    except Exception as e:
        print("Gemini Error:", e)
        return texts # fallback to original if API fails
...
def p_hi(sp):
    ...
    if texts_to_translate:
        # Send in batches of 100 to avoid AI skipping lines
        batch_size = 100
...
async def main():
    ...
    await app.start()
    try:
        await app.edit_message_text(CHAT_ID, MSG_ID, "📥 Downloading...")
        fp = await app.download_media(FILE_ID, progress=prog, progress_args=("📥 Downloading...",))
        ...
        await app.send_document(CHAT_ID, document=out, caption=cap, reply_to_message_id=MSG_ID)
